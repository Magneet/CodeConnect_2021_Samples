...
ic_domain_account = next(item for item in (config.get_ic_domain_accounts()) if item["username"] == "sa_pod02")

data["access_group_id"] = local_access_group["id"]
data["automated_farm_settings"]["customization_settings"]["ad_container_rdn"] = "OU=RDS,OU=Pod2,OU=VMware,OU=EUC"
